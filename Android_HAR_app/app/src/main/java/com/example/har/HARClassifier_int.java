...
public class HARClassifier_int {
    ...
    private static final String Model_Name_int = "tflite_model_quant_uint8.tflite";
    public int[] output_shape;
    public int output_index = 0;

    //Load Model into Interpreter
    public HARClassifier_int(Context context) throws IOException {
        MappedByteBuffer tfliteModel = FileUtil.loadMappedFile(context, Model_Name_int);
    ...
    public byte[][] predictions_int(byte[][][] input_data){
        byte[][] results = new byte[1][4]; //Output shape of results
    ...
    public float[][] LoadTest_int(Context context, String file) throws IOException {
    ...
    public float[][] LoadTestlabel_int(Context context, String file) throws IOException {
        //Read input data to form 2D array
        BufferedReader reader = new BufferedReader(new InputStreamReader(context.getAssets().open(file)));
        int rows = 100; //8526
        int columns = 1;
